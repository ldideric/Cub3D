...
/*   Updated: 2020/10/19 22:47:16 by ldideric      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int					load_img(void)
{
	...
	img = mlx_xpm_file_to_image(g_vars.mlx, g_vars.data.b.sprites.s.path, &img_width, &img_height);
...
int					img(t_base *b, int x)
{
	...
	while (y < g_m.draw_end)
	{
		...
		unsigned int color = get_color(&g_vars.data, g_m.tex.x, g_m.tex.y);
